...
export function useMeals(options?: { enabled?: boolean }) {
  ...
  const { data: meals = [], isLoading: ml } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data as Meal[]).map(m => ({ ...m, color: colorFromName(m.id) }));
    },
    retry: 3,
    retryDelay: 500,
    enabled,
  ...
  const { data: possibleMeals = [], isLoading: pl } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .order("sort_order", { ascending: true });
      if (error) throw error;
      return (data as unknown as PossibleMeal[]).map(pm => ({
        ...pm,
        meals: pm.meals ? { ...pm.meals, color: colorFromName(pm.meals.id) } : pm.meals
      }));
    },
    retry: 3,
    retryDelay: 500,
  ...
  const addMealToPossibleDirectly = useMutation({
    mutationFn: async ({ name, category, colorSeed, calories, protein, grams, ingredients, expiration_date, possible_quantity, counter_start_date }: { name: string; category: string; colorSeed?: string; calories?: string | null; protein?: string | null; grams?: string | null; ingredients?: string | null; expiration_date?: string | null; possible_quantity?: number; counter_start_date?: string | null }) => {
