/**
 * UnParUnSection — Section "Un par un" pour viandes et féculents.
 *
 * Affiche deux colonnes (Viande / Féculent) avec les aliments typés.
 * Chaque carte montre : nom, compteur, grammage, quantité, calories,
 * protéines, date de péremption, et statut "inutilisé" (⚡).
 *
 * Fonctionnalités :
 * - Tri manuel (drag & drop + long press sur mobile) ou par péremption
 * - Dialog de consommation partielle (choisir quantité/grammage)
...
export function UnParUnSection({ category, foodItems, allMeals, collapsed, onToggleCollapse, onMoveToPossible, sortMode, onToggleSort }: UnParUnSectionProps) {
  ...
  const sortItems = (items: FoodItem[], storedOrder: string[]) => {
    ...
    return [...items].sort((a, b) => {
      ...
      // Within same group, unused first
