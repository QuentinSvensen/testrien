/**
 * MealPlanGenerator — Générateur de menu hebdomadaire.
 *
 * Génère un plan de 20 repas en optimisant l'utilisation du stock
 * avec un algorithme glouton itératif :
 * 1. Sélection greedy de 16 recettes uniques (score = usage stock + aléatoire)
 * 2. Passe de réparation : swaps pour aligner la consommation sur des
 *    multiples exacts des tailles de paquets
 * 3. Ajout de "Avant grimpe" × N (configurable)
 *
 * Fonctionnalités :
 * - Détection automatique des groupes "Surgelés" et "Toujours présent"
 * - Synchronisation des coches vertes dans la liste de courses
 * - Persistance du menu généré et des besoins en ingrédients (préférences)
 * - Tri par calories, alphabétique ou manuel
 * - Liste d'ingrédients avec statut ✅ / ❌ / ❓
 *
 * getRecipeUsage() : extrait la consommation d'un repas (grammes/quantité par ingrédient)
 * scoreRecipe() : note un candidat selon le stock restant et la variété
 */
...
const MENU_MAIN_COUNT_KEY = "menu_generator_main_count_v1";
...
export function MealPlanGenerator() {
  ...
  const persistedMainCount = getPreference<number>(MENU_MAIN_COUNT_KEY, 16);
  const [mainMealCount, setMainMealCount] = useState<number>(16);

  useEffect(() => {
    if (typeof persistedGrimpeCount === 'number') setGrimpeCount(persistedGrimpeCount);
  }, [persistedGrimpeCount]);
  useEffect(() => {
    if (typeof persistedMainCount === 'number') setMainMealCount(persistedMainCount);
  }, [persistedMainCount]);
  useEffect(() => {
  ...
  // Identifier les IDs de groupe de produits surgelés (surgelés/congelés)
  ...
  // Identifier les IDs de groupe "Toujours présent"
  ...
  // Construire l'ensemble des clés normalisées pour les aliments "Toujours présent"
  ...
  const updateShoppingChecks = (needsMap: Map<string, { grams: number; count: number; rawName?: string }>) => {
    ...
    for (const [needKey, need] of needsMap) {
      ...
      const matchName = need.rawName || needKey; // Utiliser le nom brut pour une correspondance sans accents
      ...
      // Cocher automatiquement uniquement si correspondance exacte OU une seule correspondance partielle
      // Plusieurs correspondances partielles → laisser décoché pour le système ❓ coloré
  ...
  useEffect(() => {
    ...
    // Utiliser sessionStorage pour survivre aux changements d'onglet (remontages du composant)
  ...
  const generatePlan = () => {
    ...
    // Suivre la consommation cumulative par ingrédient pour toutes les recettes sélectionnées
    const totalUsage = new Map<string, { grams: number; count: number }>();

    // Mélanger les candidats pour une meilleure diversité
    const shuffled = [...candidatePlats].sort(() => Math.random() - 0.5);

    // Aide : trouver la clé d'inventaire correspondante
    ...
    // Noter à quel point la consommation cumulative est proche d'un multiple entier de la taille du paquet
    const multipleScore = (cumGrams: number, pkgGrams: number): number => {
      ...
      const ratio = dist / pkgGrams; // 0 = multiple parfait, 0.5 = pire
      if (ratio < 0.02) return 15;   // multiple exact → bonus massif
      if (ratio < 0.1) return 8;     // proche d'un multiple
      if (ratio < 0.2) return 2;     // acceptable
      return -ratio * 12;            // pénalité très forte (ex: pot de 300g utilisable par 200g → -6)
    };

    // Greedy selection: mainMealCount recipes optimizing for whole-multiple consumption
    // Priorité forte: fermer les ingrédients en grammes entamés
    const mainTarget = mainMealCount;
    for (let i = 0; i < mainTarget; i++) {
      ...
      const scoreRecipe = (recipe: typeof shuffled[0]) => {
        ...
        if (hasInventory) {
          ...
          for (const [ingKey, used] of usage) {
            ...
            if (used.grams > 0 && inv.pkgGrams > 0) {
              ...
              if (newCumGrams > inv.grams * 1.05) {
                ...
                // Léger bonus pour fermer un ingrédient entamé
          ...
          // Préférence modérée pour les recettes utilisant des ingrédients entamés
          if (openGramKeys.size > 0 && touchesOpen) score += 2;
        }

        // Facteur aléatoire important pour un maximum de variété
        score += Math.random() * 8;
        return score;
      };
      ...
      if (uniquePool.length > 0) {
        ...
        // Choisir parmi les 6 meilleurs pour plus de diversité
        ...
        // Règle spéciale pour "Pizza Pizzeta" : toujours par paire
        const pickedMeal = candidatePlats.find(r => r.id === pick.id);
        const nameLower = pickedMeal?.name.toLowerCase() || "";
        if (nameLower.includes("pizza pizzeta")) {
          selectedIds.push(pick.id);
          counts.set(pick.id, 2);
          i++; // Prend un créneau supplémentaire pour atteindre l'objectif principal (environ 20 au total)
        }
      } else {
        break;
      }
      ...
      if (recipe && hasInventory) {
        ...
        for (const [ingKey, used] of usage) {
          ...
          totalUsage.set(matchKey, {
            grams: prev.grams + used.grams * multiplier,
            count: prev.count + used.count * multiplier,
          });
        }
      }
    }

    // ── Passe de réparation : forcer des multiples exacts des tailles de paquets ──
      ...
      for (const mis of misaligned) {
        ...
        for (let idx = 0; idx < mainCount; idx++) {
          ...
          for (const cand of candidatePlats) {
            ...
            // Ne pas introduire de nouveaux doublons pendant les échanges de réparation
    ...
    // Construire la carte des besoins totaux pour la persistance des coches de courses
    ...
    // Persister les besoins pour une re-vérification lors de l'activation du bouton vert
    ...
    // Réinitialiser l'indicateur de synchronisation de session pour que le nouveau menu prenne effet
    sessionStorage.removeItem(SESSION_SYNC_KEY);
    // Mettre à jour les cases à cocher et les quantités de la liste de courses
  ...
  const shoppingItems2 = useMemo(() => {
    ...
    // Vérifier quels ingrédients correspondent aux articles de la liste de courses ou aux aliments "Toujours présent"
    const toujoursKeys = [...toujoursFoodKeys];
    for (const [key, item] of map) {
      ...
      if (partialMatches.length === 1) {
        // Correspondance partielle unique → trouvé (ex: "Épices" correspond à "Épices fajitas")
        item.matched = true;
      } else if (partialMatches.length > 1) {
        // Plusieurs correspondances partielles → ambigu (❓)
        item.matched = true;
        item.ambiguous = true;
      }
      // 0 correspondance → reste non trouvé (❌)
    }

    return Array.from(map.entries())
      .map(([, v]) => v)
      .sort((a, b) => {
        // Ambigu (❓ bleu) en premier, puis non trouvé (❌ rouge), puis trouvé
  ...
  // Séparer "Avant grimpe" des repas principaux
    ...
    <div className="max-w-4xl mx-auto space-y-4">
          ...
          <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setMainMealCount(val);
                setPreference.mutate({ key: MENU_MAIN_COUNT_KEY, value: val });
              }}
      ...
      {selectedMeals.length === 0 ? (
        <p className="text-muted-foreground text-sm text-center py-8 italic">
          Clique sur "Générer" pour créer un menu aléatoire de {mainMealCount + grimpeCount} plats
        </p>
      ) : (
        <>
          <div className="rounded-3xl bg-card/80 backdrop-blur-sm p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {(() => {
                // Ordre par colonne : tous les articles dans col1, puis col2
                ...
                const renderMealCard = (meal: Meal, prefix: string, i: number) => {
                  ...
                  >
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {displayCal && (
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shrink-0 ${ingCal !== null ? 'bg-orange-500/50 text-white font-bold' : 'text-white/70 bg-white/20'}`}>
                          <Flame className="h-2.5 w-2.5" />{displayCal}
                        </span>
                      )}
                      {displayPro && (
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full flex items-center gap-0.5 shrink-0 font-semibold ${ingPro !== null ? 'bg-blue-600/60 text-white' : 'text-white/70 bg-blue-500/30'}`}>
                          🍗 {displayPro}
                        </span>
                      )}
                      {meal.grams && (
